...
class OnboardScreenTest {
  ...
  Future<void> init() async {
    ...
    expect(find.byKey(const Key("onboardStepperKey")), findsOneWidget);

    await _goToProfileSetup();
    await _goToTutorialScreen();
    
    if (Platform.isIOS) {
      await tester.tap(find.byKey(const Key("stepperButtonKey")).at(3));
  ...
  Future<void> initLogin() async {
    await tester.tap(find.byKey(const Key("stepperButtonKey")).at(2));
  ...
  Future<void> _goToProfileSetup() async {
    await tester.tap(find.byKey(const Key("stepperButtonKey")).first);
    await tester.pumpAndSettle();

    await tester.tap(find.byKey(const Key("stepperButtonKey")).at(1));
    await tester.pumpAndSettle();
  ...
  Future<void> _finishOnboarding() async {
    await tester.tap(find.byKey(const Key("stepperButtonKey")).last);
    await tester.pumpAndSettle();
  }
}