...
class TipScreenTest {
  ...
  Future<void> _changeDefaultTipError() async {
    ...
    await tester.enterText(find.byKey(Key("defaultTipFieldKey")), "2t");
    await tester.pump(Duration(milliseconds: 300));
    expect(find.text('Must be between 0 and 30'), findsOneWidget);

    await tester.enterText(find.byKey(Key("defaultTipFieldKey")), "17");
    await tester.pump(Duration(milliseconds: 300));
    expect(find.text('Must be between 0 and 30'), findsNothing);
  }
  ...
  Future<void> _changeQuickTipError() async {
    ...
    await tester.enterText(find.byKey(Key("quickTipFieldKey")), "1^");
    await tester.pump(Duration(milliseconds: 300));
    expect(find.text('Must be between 0 and 20'), findsOneWidget);

    await tester.enterText(find.byKey(Key("quickTipFieldKey")), "17");
    await tester.pump(Duration(milliseconds: 300));
    expect(find.text('Must be between 0 and 20'), findsNothing);
  ...
  Future<void> _submitError() async {
    expect(find.byKey(Key("tipFormSnackbarKey")), findsNothing);

    await tester.tap(find.byKey(Key("submitButtonKey")));
    await tester.pumpAndSettle();
    expect(find.byKey(Key("tipFormSnackbarKey")), findsOneWidget);

    await tester.fling(find.byKey(Key("tipFormSnackbarKey")), Offset(0, 500), 500);
    await tester.pump();
  }

  Future<void> _changeDefaultTipSuccess() async {
    await tester.enterText(find.byKey(Key("defaultTipFieldKey")), "22");
    await tester.pump(Duration(milliseconds: 300));
  }
