...
class EditPhoto extends StatelessWidget {
  ...
  const EditPhoto({bool autoDismiss = false, Key? key})
    ...
    return BlocListener<EditPhotoBloc, EditPhotoState>(
      ...
      child: BlocBuilder<EditPhotoBloc, EditPhotoState>(
        builder: (context, state) {
          return Stack(
            children: [
              _createAvatar(profile: profile, state: state),
              Positioned(
                bottom: 0.0,
                right: -10.0,
                child: RawMaterialButton(
                  key: const Key("addPhotoButtonKey"),
                  onPressed: () => _editPhoto(profile: profile, context: context, state: state),
  ...
  Widget _createAvatar({required Profile profile, required EditPhotoState state}) {
    if (state is Submitting || state is SubmitSuccess) {
    ...
    } else if (profile.photos.largeUrl.isEmpty) {
      return CircleAvatar(
      ...
      return CachedAvatar(
        url: profile.photos.largeUrl,
        radius: 100.sp,
      );
    }
  }

  void _editPhoto({required BuildContext context, required Profile profile, required EditPhotoState state}) async {
    if (state is !Submitting) {
      if (context.read<IsTestingCubit>().state) {
        XFile file = await const MockImagePicker().init();
        BlocProvider.of<EditPhotoBloc>(context).add(ChangePhoto(profileIdentifier: profile.identifier, photo: file));
        return;
      }

      final bool permissionGranted = await _requestPermission();
      if (permissionGranted) {
        XFile? photo = await RepositoryProvider.of<PhotoPickerRepository>(context).pickPhoto();
        if (photo != null) {
          BlocProvider.of<EditPhotoBloc>(context).add(ChangePhoto(profileIdentifier: profile.identifier, photo: photo));
        }
      } else {
